CREATING A DATABASE SQL AND MYSQL

Q. What is database??
-->> Database is an organized way of collecting data or a type of data store based on the use of a DBMS..
...
-->> Most commonly used databases are: 1. Relational DB AND  2. NoSQL DB

🔹 What is RDBMS?

RDBMS (Relational Database Management System) is a software system used to store, manage, and retrieve structured data in tables.
...
.ACID compliant (high reliability)
...
NoSQL (Not Only SQL) databases are used to store unstructured or semi-structured data.

🧠 Key Idea:

.Not limited to tables (unlike RDBMS)
.Flexible schema (no fixed structure)
.Designed for scalability & high performance

--->>> RDBMS(MYSQL) VS NOSQL(MONGODB)
follow this link for comparison table

![alt text](ssql-vs-nosql.png).